...
- **协作习惯**：倾向于利用 AI 进行初稿生成、创意启发、最终产物输出；在将元设计空间转化为 AI 生成规则方面有经验，并与 AI 保持协作共创关系

### 2.2 用户需求

- **高效**：减少反复试错与修改成本
- **高质量**：生成结果稳定、符合预期方向、可用于实际设计任务推进
...
然而，当前协作模式仍存在**元设计空间难以被清晰界定并稳定配置**的问题，导致 AI 对设计师需求产生理解偏差，协作过程需要反复试错与修改，效率较低，生成结果也难以稳定符合设计师预期。

更具体地，信息类设计中的“元设计空间”往往不是单一维度的审美偏好，而是由多个层面共同构成：
...
当这些要素只能被压缩为一段自然语言输入时，表达会不可避免地出现“模糊、遗漏、冲突、难复现”的问题：模型只能推断隐含要求，难以保持结构稳定与风格延续，设计师则通过反复改写 Prompt、反复抽卡与手动修补来补偿，从而造成效率低下与质量不稳定。
...
flowchart TD
    ...
    B --> C[GenAI interprets the implicit meta-design space]
...
```mermaid
flowchart TD
    A[Designer] --> B[Defines meta-design space]
...
**核心研究问题：**\
如何基于元设计理论，重构设计师与生成式 AI 的协作模式，提升设计师在信息设计场景下构建与调用元设计空间的能力和交互体验，并最终提高生成内容的质量与效率？
...
### 2. 为什么“元设计空间的外化与配置”在信息类设计场景是一个大痛点？

<br />

### 3. 随着模型能力进化，这个课题是否仍然有价值？

- **元设计空间的外化与配置仍是瓶颈**：即使模型能力增强，设计师如何准确构建并调用元设计空间仍是关键问题
