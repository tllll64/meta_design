...
在本研究设想中，元设计机制至少包含三类关键能力：
1) **意图结构化**：将设计意图从自然语言抽象为结构化对象（内容原子、结构层级、关系约束、正例/反例等）。
2) **规则与过程可配置**：允许设计师定义并调整生成规则、约束与流程节点顺序（内容/结构/视觉可非线性编排）。
3) **可对照迭代**：把生成偏差定位到具体规则/关系/结构节点，支持回写并形成版本快照，减少盲目试错。
...
- **意图天然是多维组合**：信息类设计需要同时对齐目标、内容取舍、结构逻辑与形式呈现；这些维度之间还存在依赖与权衡，难以用单段 Prompt 充分覆盖。
- **结构稳定性要求更高**：信息传达以层级与逻辑为核心，一旦结构理解偏差，局部修补难以挽回整体阅读路径与叙事关系。
- **可控与可复现更关键**：设计任务常需要延续同一套结构与风格到不同版本/不同项目；纯 Prompt 难以沉淀为可复用资产，导致“偶尔满意但难复现”。
- **迭代成本集中在表达与归因**：当前协作中，设计师往往知道“哪里不对”，但很难把问题归因到可执行的规则/关系/流程节点，最终只能以抽卡式试错消耗时间。
