...
export const slides: Slide[] = [
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          '核心痛点：信息设计场景中，设计意图包含内容组织、层级关系、视觉风格、呈现形式与人机分工等多个维度。',
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          'RQ2：哪些体现设计意图的关键内容适合被显式化、结构化，并转化为可配置的生成规则？',
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          '元设计维度提炼：提炼设计意图中适合被元设计化的关键维度（如目标、内容、结构、形式、流程与分工），并探索其与生成规则的映射关系。',
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          '问题建构：强调设计意图不仅是风格偏好，还包括内容、结构、流程与人机分工等多维因素，并尝试结构化表达。',
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'figure', title: '图：设计师与生成式 AI 的协作模式框架', note: '核心环节：意图表达、规则配置、系统生成、人工微调' },
      {
        type: 'bullets',
        items: [
          '角色重塑：设计师从“执行者”转变为“规则制定者（元设计师）”',
          'AI定位：从“单向工具”转变为“基于约束的协同创作者”',
          '核心机制：通过结构化提取设计意图，形成可复用的生成规则，实现“先定义规则、再生成结果”',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    title: '信息设计场景下设计意图元设计内容构建与生成规则映射研究',
    todo: true,
    blocks: [
      {
        type: 'text',
        variant: 'body',
        text: '结合前期的案例分析与用户访谈，针对研究问题 2（RQ2），我们对设计意图的元设计内容与生成规则的映射关系进行了如下构建：',
      },
      { type: 'figure', title: '图：设计意图到生成规则的映射模型', note: '核心维度：目标、内容、结构、形式、流程与分工' },
      {
        type: 'bullets',
        items: [
          '意图解构：将模糊的自然语言指令拆解为可被系统理解的元设计维度',
          '规则映射：探索各维度意图如何转化为系统可执行的具体生成规则',
    ...
    blocks: [
      ...
      { type: 'figure', title: '图：系统架构与核心交互界面草图', note: '包含：React 前端 + Node.js 后端，以及规则配置面板' },
      {
        ...
        items: [
          ...
          '意图转化：尝试把设计意图中的关键维度转化为可编辑、可约束、可复用的系统输入。',
